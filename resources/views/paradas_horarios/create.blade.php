...
@section('content') 
    ...
    <div>
        ...
        <div class="row">        
          ...
          <form action="{{url('/admin/paradas')}}/{{$parada->id}}/horario/{{$horario->id}}"" method="post" class="form-content fadeIn animated">
            {{ csrf_field() }}
                    ...
                    <div>
                        ...
                        <div class="row">
                            <div class="col-md-12">
                                ...
                                <div id="parada-horario-{{$parada->id}}" class="parada-drag parada-new" parada="{{$parada->id}}" draggable="true" ondragstart="drag(event)">
                                        {{$parada->nombre}}

                                        <input type="hidden" name="parada" value="">
                                        <input type="hidden" name="ruta" value="{{$ruta->id}}">

                                        <!-- <input type="hidden" name="paradas_horario[]" value="0"> -->
                                </div>
...
@section('scripts')
    <script type="text/javascript">
        ...
        function checkDropParadaActual(elemento){

            var parada_puesta = $('#parada_posicion');

            if(parada_puesta.length <= 0){

                var attr = $("#"+elemento).attr('parada');

                // For some browsers, `attr` is undefined; for others, `attr` is false. Check for both.
                if (typeof attr !== typeof undefined && attr !== false) {

                    $("input[name='parada']").val(attr);

                    $("#"+elemento).append('<input id="parada_posicion" type="hidden" name="paradas_horario[]" value="0">');
                }
    ...
    <script>
        ...
        function getIcon(color){
            ...
            console.log(window.location.host+"/public/images/blue-pin.png");
